...
it("renders without crashing with enzyme", () => {
  const wrapper = shallow(<LogosContainer {...mockProps} />);
  expect(wrapper.exists()).toBe(true);
});

describe("logo click", () => {
  it("displays the hex on click", () => {
    const wrapper = shallow(<LogosContainer {...mockProps} />);
    expect(wrapper.find(Logo).length).toBe(2);
    const logo = wrapper.childAt(0);

    logo.simulate("click");

    expect(wrapper.find(Logo).length).toBe(1);
    const hex = wrapper.childAt(0);
    expect(hex.childAt(0).text()).toContain("#");
  });
});
